...
def sheetsNumber(fileName):
    fileObject = xlrd.open_workbook(f"{fileName}", on_demand = True)            
    return len(fileObject.sheet_names())


def fileStream(fileName,index):
    fileObject = xlrd.open_workbook(f"{fileName}", on_demand = True)   
    currentSheet = fileObject.sheet_by_index(index) #sheetIndex
...
def emptyOnLeftCell(row,col,sheet):
    isEmpty = False    
    if((sheet.cell_type(row,col) == xlrd.XL_CELL_EMPTY and (sheet.cell_type(row,1) != xlrd.XL_CELL_EMPTY or sheet.cell_type(row,2) != xlrd.XL_CELL_EMPTY)) 
...
def fileRead(sheet):    
    ...
    for row in range(3,sheet.nrows): 
        ...
        for col in range(0,sheet.ncols):
            ...
            if row >= 4 and col <= 2: #Wiersz
                emptyLeftCell = emptyOnLeftCell(row,col,sheet)
                print(emptyRow," ",emptyLeftCell)

                if emptyRow == True or  emptyLeftCell == True:
                    print(colored((row+1,col+1,"Pominięte"),"yellow"))
                    continue

                structs.UP_SchematyOcenOpisowychWiersz.kolejnosc = row-3 if col == 0 else row-2 if col == 1 else row-1
                structs.UP_SchematyOcenOpisowychWiersz.wartosc = sheet.cell_value(row,col).strip().upper() if col == 0 else sheet.cell_value(row,col).strip().capitalize() + ":" if (col == 1 and (sheet.cell_type(row,col) != xlrd.XL_CELL_EMPTY)) else sheet.cell_value(row,col).strip()
                structs.UP_SchematyOcenOpisowychWiersz.wyroznienie = 1 if (col == 0 or col == 1) else 0
